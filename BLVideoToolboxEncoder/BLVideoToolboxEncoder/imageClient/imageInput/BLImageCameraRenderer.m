...
(
 ...
 void main()
 {
    ...
    yuv.yz = textur2D(chrominanceTexture, textureCoordinate).ra - vec2(0.5, 0.5);
    rgb = colorConversionMatrix * yuv;
...
@implementation BLImageCameraRenderer{
    BLImageProgram *program;
...
- (BOOL)prepareRender:(BOOL)isFullYUVRange{
    BOOL ret = FALSE;
    if (isFullYUVRange) {
        program = [[BLImageProgram alloc] initWithVertexShaderString:vertexShaderString
                                                fragmentShaderString:fragmentShaderString];
    } else {
        program = [[BLImageProgram alloc] initWithVertexShaderString:vertexShaderString
                                                fragmentShaderString:YUVVideoRangeConversionForLAFragmentShaderString];
    }
...
- (void)convertYUVToRGBOutputWithWidth:(int)bufferWidth
                           ...
                           aspectRatio:(float)aspectRatio
                   preferredConversion:(const GLfloat *)preferredConversion
                      inputTexRotation:(BLImageRotationMode)inputTexRotation{
    ...
    glVertexAttribPointer(positionAttribute,
                          ...
                          squareVertices);
    glEnableVertexAttribArray(positionAttribute);
    glVertexAttribPointer(textureCoordinateAttribute,
                          2,
                          GL_FLOAT,
                          0,
                          0,
